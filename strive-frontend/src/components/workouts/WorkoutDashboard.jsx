...
const WorkoutDashBoard = ({ user, workouts, startWorkout, suggestions }) => {
    const lastThreeWorkouts = workouts.length > 0 ? [...workouts].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 3) : []

    return (
        <section className="space-y-5 w-full text-[#EDF2F4]">
            <div className="text-4xl md:text-6xl font-semibold text-center p-4">
            ...
            <div className="bg-[#8D99AE] rounded-2xl p-5">
                <h1 className="text-center text-2xl font-bold mb-5">
                    Trainer <span className="text-[#EF233C]">Suggestions</span>
                </h1>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center justify-center">
                    {suggestions.map((suggestion) => (
                        <div key={suggestions._id} className="flex flex-col flex-1 rounded-2xl shadow-xl p-2 bg-[#2B2D42] transition hover:scale-102">
                            <h1 className="font-bold text-xl text-center text-[#EF233C] border-b border-[#EDF2F4]/40 p-2 mb-2">
                                {suggestion.title}
                            </h1>
            ...
            <div className="card-theme fade-in-card p-6 w-full mx-auto bg-[#8D99AE] rounded-2xl" style={{ animationDelay: '0.4s' }}>
                ...
                <button onClick={startWorkout} className="w-full bg-[#EF233C] py-2 px-4 rounded-xl hover:bg-[#D90429]">
                    Start Workout
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 w-full gap-5">
                <div className="fade-in-card flex flex-col items-center w-full space-y-5 p-2 bg-[#8D99AE] rounded-2xl" style={{ animationDelay: '0.8s' }}>
