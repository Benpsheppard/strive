	...
	return (
		<div className="bg-[#8D99AE] p-6 rounded-2xl">
			...
			<div className="flex gap-2 mb-6 justify-center flex-wrap">
				...
				{/* Sets View Mode */}
				<button className={`px-4 py-2 rounded-lg font-medium transition-COLOURS ${viewMode === 'sets' ? 'bg-[#EF233C] text-[#EDF2F4]' : 'bg-[#2B2D42] text-[#EDF2F4] hover:bg-opacity-80'}`} onClick={() => setViewMode('sets')}>
					By Sets
				</button>

				{/* Weight View Mode */}
				<button className={`px-4 py-2 rounded-lg font-medium transition-COLOURS ${viewMode === 'weight' ? 'bg-[#EF233C] text-[#EDF2F4]' : 'bg-[#2B2D42] text-[#EDF2F4] hover:bg-opacity-80'}`} onClick={() => setViewMode('weight')}>
					By Weight
				</button>
			</div>
